...
import { ListTodo, TrendingUp, CheckCircle2, FolderOpen, Plus, Briefcase, ChevronRight } from "lucide-react"
...
async function getDashboardData(workspaceIdParam?: string) {
  ...
  if (!activeId) {
    return { workspaces: workspaces ?? [], activeId, activeWorkspace, projects: [], tasks: [] }
  }

  // Parallel fetch projects + tasks with error handling
  const [projResult, tasksResult] = await Promise.all([
    supabase.from("projects").select("*").eq("workspace_id", activeId).order("name"),
    supabase
      .from("tasks")
      .select("id, project_id, title, status, created_at, projects!inner ( id, workspace_id )")
  ...
  return {
    ...
    activeId,
    activeWorkspace,
    projects: projResult.data as Database["public"]["Tables"]["projects"]["Row"][],
    tasks: tasksResult.data as unknown as Pick<Database["public"]["Tables"]["tasks"]["Row"], "id" | "project_id" | "status">[],
  }
}
...
export default async function DashboardPage({ searchParams }: PageProps) {
  ...
  const { workspaces, activeId, activeWorkspace, projects, tasks } = await getDashboardData(workspaceId)
          ...
          <div className="space-y-10">
            ...
            <div>
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <StatCard label="To Do" value={todoCount} description="Tasks pending assignment or start" icon={ListTodo} accentColor="amber" />
                <StatCard label="In Progress" value={inProgressCount} description="Tasks currently being worked on" icon={TrendingUp} accentColor="blue" />
                <StatCard
                  label="Completed"
                  value={doneCount}
            ...
            {/* Projects */}
            <ProjectsSection projects={projects} tasks={tasks} activeId={activeId} />
          </div>
        )}
      </div>
    </AppShell>
...
function EmptyState() {
  return (
    <Card className="border-dashed border-zinc-300 dark:border-zinc-800 bg-transparent flex flex-col items-center justify-center p-12 text-center max-w-xl mx-auto mt-12">
      ...
      <CardTitle className="text-xl font-bold mb-2 text-zinc-900 dark:text-white">No Workspace Found</CardTitle>
      <CardDescription className="text-zinc-500 max-w-sm mb-6 dark:text-zinc-400">
        You aren&apos;t a member of any workspaces yet. Create your first workspace to start collaborating.
      </CardDescription>
      <CreateWorkspaceTrigger />
    </Card>
  )
...
function ProjectsSection({
  ...
  tasks: Pick<Database["public"]["Tables"]["tasks"]["Row"], "id" | "project_id" | "status">[]
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold tracking-tight text-zinc-800 dark:text-zinc-100">Active Projects</h2>
        </div>
        {activeId && (
          <Link
            href={`/workspace/${activeId}`}
            className={cn(buttonVariants({ variant: "ghost", size: "sm" }), "text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white")}
          >
            Manage Projects
      ...
      {projects.length === 0 ? (
        <Card className="border-dashed border-zinc-300 dark:border-zinc-800 bg-transparent flex flex-col items-center justify-center p-8 text-center">
          <CardDescription className="text-zinc-500 mb-4 dark:text-zinc-400">
            No projects found in this workspace. Create a project to start organizing tasks.
          </CardDescription>
          <Link
            href={`/workspace/${activeId}`}
            className={cn(buttonVariants({ size: "sm" }), "bg-zinc-900 hover:bg-zinc-800 text-white dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200")}
          >
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((proj) => {
            ...
            return <ProjectCard key={proj.id} project={proj} doneCount={pt.filter((t) => t.status === "done").length} totalCount={pt.length} />
          })}
        </div>
      )}
    </div>
